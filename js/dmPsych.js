const dmPsych = (function() {
  ...
  */
  ...
  obj.spinner = function(canvas, spinnerData, sectors, effort, guaranteedOutcome, nSpins) {
    ...
    let readyToSpin = false;     // true when the wheel is ready to spin
    ...
    const getStim = () => {
      stim_charCode = 65 + Math.floor(Math.random() * 16 + 5);
      stim_char = String.fromCharCode(stim_charCode);
      console.log(stim_charCode, stim_char);
      let stim_html = `<span style="color:black; font-size: 25px">${stim_char}</span>`
      return stim_html
    };
    ...
    const displayStim = () => {
      if (readyToSpin) {
        ...
        nPress == 0 ? pointer.innerHTML = getStim() : pointer.innerHTML;
    ...
    const listenForInput = (e) => {
      if (readyToSpin) {
        ...
        vel = 0;
        vel_postDecel = 0;
        if (effort == "lowEffort" && e.keyCode == stim_charCode + 1 + nCorrect || effort == "highEffort" && e.keyCode == stim_charCode - 1 - nCorrect) {
          nCorrect++;
          isPushed = true;
          if (nCorrect == 5) {
            readyToSpin = true;
